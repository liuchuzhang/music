...
export class netease {
  ...
  static weapi(text) {
    ...
    const enc_text = btoa(
      this._aes_encrypt(btoa(this._aes_encrypt(text, nonce, 'AES-CBC').data), sec_key, 'AES-CBC').data
    )
  ...
  static ne_show_toplist(offset) {
    ...
    const url = '/api/music163/weapi/toplist/detail'
        ...
        axios.post(url, new URLSearchParams(data)).then((response) => {
          ...
          response.data.list.forEach((item) => {
            const playlist = {
              ...
              source_url: `/api/music163/#/playlist?id=${item.id}`,
              title: item.name
            }
  ...
  static show_playlist(url) {
    ...
    let target_url = ''
    if (offset != null) {
      target_url = `/api/music163/discover/playlist/?order=${order}${filter}&limit=35&offset=${offset}`
    } else {
      target_url = `/api/music163/discover/playlist/?order=${order}${filter}`
    }
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const list_elements = Array.from(
            new DOMParser().parseFromString(data, 'text/html').getElementsByClassName('m-cvrlst')[0].children
          )
          const result = list_elements.map((item) => ({
            ...
            source_url: `/api/music163/#/playlist?id=${getParameterByName(
              'id',
              item.getElementsByTagName('div')[0].getElementsByTagName('a')[0].href
            )}`
          }))
  ...
  static ng_render_playlist_result_item(index, item, callback) {
    const target_url = '/api/music163/weapi/v3/song/detail'
    ...
    axios.post(target_url, new URLSearchParams(data).toString()).then((response) => {
      ...
      const track = {
        ...
        source: 'netease',
        source_url: `/api/music163/#/song?id=${track_json.id}`,
  ...
  static ne_get_playlist(url) {
    ...
    return {
      success: (fn) => {
        ...
        const target_url = '/api/music163/weapi/v3/playlist/detail'
        const d = {
          id: list_id,
          offset: 0,
          total: true,
        ...
        axios.post(target_url, new URLSearchParams(data)).then((response) => {
          ...
          const info = {
            ...
            source_url: `/api/music163/#/playlist?id=${list_id}`
          }
  ...
  static bootstrap_track(track, success, failure) {
    console.log('bootstrap_track', track)
    const sound = {}
    const target_url = `/api/interface3/eapi/song/enhance/player/url`
  ...
  static search(url) {
    ...
    const target_url = '/api/music163/api/search/pc'
          ...
          .then((response) => {
            ...
            if (searchType === '0') {
              result = data.result.songs.map((song_info) => ({
                ...
                source: 'netease',
                source_url: `/api/music163/#/song?id=${song_info.id}`,
              ...
              result = data.result.playlists.map((info) => ({
                ...
                title: info.name,
                source: 'netease',
                source_url: `/api/music163/#/playlist?id=${info.id}`,
  ...
  static ne_album(url) {
    ...
    // use chrome extension to modify referer.
    const target_url = `/api/music163/api/album/${album_id}`
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `/api/music163/#/album?id=${data.album.id}`
          }
  ...
  static ne_artist(url) {
    ...
    // use chrome extension to modify referer.
    const target_url = `/api/music163/api/artist/${artist_id}`
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `/api/music163/#/artist?id=${data.artist.id}`
          }
  ...
  static lyric(url) {
    ...
    // use chrome extension to modify referer.
    const target_url = '/api/music163/weapi/song/lyric?csrf_token='
    const csrf = ''
    const d = {
      id: track_id,
      lv: -1,
  ...
  static login(url) {
    // use chrome extension to modify referer.
    let target_url = '/api/music163/weapi/login'
    ...
    if (loginType === 'email') {
      ...
      target_url = `/api/music163/weapi/login/cellphone`
  ...
  static get_user_playlist(url, playlistType) {
    ...
    const target_url = '/api/music163/api/user/playlist'
  ...
  static get_recommend_playlist() {
    const target_url = '/api/music163/weapi/personalized/playlist'
  ...
  static get_user() {
    const url = `/api/music163/api/nuser/account/get`
