...
export class qq {
  ...
  static get_toplist_url(id, period, limit) {
    return `/api/uyqq/cgi-bin/musicu.fcg?format=json&inCharset=utf8&outCharset=utf-8&platform=yqq.json&needNewCode=0&data=${encodeURIComponent(
      JSON.stringify({
        comm: {
          cv: 1602,
          ct: 20
  ...
  static qq_get_playlist(url) {
    ...
    return {
      success: (fn) => {
        const target_url =
          '/api/iyqq/qzone-music/fcg-bin/fcg_ucc_getcdinfo_' +
  ...
  static qq_album(url) {
    ...
    return {
      success: (fn) => {
        const target_url =
          '/api/iyqq/v8/fcg-bin/fcg_v8_album_info_cp.fcg' +
  ...
  static qq_artist(url) {
    ...
    return {
      success: (fn) => {
        const target_url = `/api/uyqq/cgi-bin/musicu.fcg?format=json&loginUin=0&hostUin=0inCharset=utf8&outCharset=utf-8&platform=yqq.json&needNewCode=0&data=${encodeURIComponent(
          JSON.stringify({
            comm: {
              ct: 24,
              cv: 0
        ...
        axios.get(target_url).then((response) => {
          ...
          const tracks = data.singer.data.songlist.map((item) => this.qq_convert_song2(item))
          return fn({
            tracks,
            info
          })
  ...
  static search(url) {
    ...
    const target_url = '/api/uyqq/cgi-bin/musicu.fcg'
    ...
    return {
      success: (fn) => {
        ...
        axios.post(target_url, query).then((response) => {
          ...
          let total = 0
          if (searchType === '0') {
            result = data.req.data.body.song.list.map((item) => this.qq_convert_song2(item))
  ...
  static lyric(url) {
    ...
    const target_url =
      '/api/iyqq/lyric/fcgi-bin/fcg_query_lyric_new.fcg?' +
  ...
  static get_user_by_uin(uin, callback) {
    const infoUrl = `/api/uyqq/cgi-bin/musicu.fcg?format=json&&loginUin=${uin}&hostUin=0inCharset=utf8&outCharset=utf-8&platform=yqq.json&needNewCode=0&data=${encodeURIComponent(
      JSON.stringify({
        comm: { ct: 24, cv: 0 },
        vip: {
