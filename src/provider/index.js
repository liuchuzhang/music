...
const MediaService = {
  ...
  insertTrackToMyPlaylist(id, track, to_track, direction) {
    const newPlaylist = myplaylist.insert_track_to_myplaylist(id, track, to_track, direction)
    return {
      success: (fn) => fn(newPlaylist)
    }
  },
  ...
  insertMyplaylistToMyplaylists(playlistType, playlistId, toPlaylistId, direction) {
    const newPlaylists = myplaylist.insert_myplaylist_to_myplaylists(playlistType, playlistId, toPlaylistId, direction)
    return {
  ...
  bootstrapTrack(track, playerSuccessCallback, playerFailCallback) {
    ...
    function failureCallback() {
      ...
      const failover_source_list = ['kuwo', 'qq', 'migu'].filter((i) => i !== trackPlatform)
