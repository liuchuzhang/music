...
export class migu {
  ...
  static mg_render_tracks(url, page, callback) {
    ...
    switch (playlist_type) {
      case 'mgplaylist':
        tracks_url = `/api/appcnfmigu/MIGUM2.0/v1.0/user/queryMusicListSongs.do?musicListId=${list_id}&pageNo=${page}&pageSize=50`;
        break;
      case 'mgalbum':
        tracks_url = `/api/appcnfmigu/MIGUM2.0/v1.0/content/queryAlbumSong?albumId=${list_id}&pageNo=${page}&pageSize=50`;
        break;
      default:
        break;
    }
  ...
  static mg_show_toplist(offset) {
    ...
    const url =
      '/api/appcnfmigu/MIGUM3.0/v1.0/template/rank-list/release?dataVersion=1616469593718&templateVersion=9';
    return {
      success: (fn) => {
  ...
  static show_playlist(url) {
    ...
    let target_url = '';
    if (!filterId) {
      target_url = `/api/appcnfmigu/MIGUM2.0/v2.0/content/getMusicData.do?count=${pageSize}&start=${
        offset / pageSize + 1
      }&templateVersion=5&type=1`;
    } else {
      target_url = `/api/appcnfmigu/MIGUM3.0/v1.0/template/musiclistplaza-listbytag?pageNumber=${
        offset / pageSize + 1
  ...
  static mg_get_playlist(url) {
    ...
    return {
      success: (fn) => {
        const info_url = `/api/appcnfmigu/MIGUM2.0/v1.0/content/resourceinfo.do?needSimple=00&resourceType=2021&resourceId=${list_id}`;
  ...
  static mg_album(url) {
    ...
    return {
      success: (fn) => {
        const info_url = `/api/appcnfmigu/MIGUM2.0/v1.0/content/resourceinfo.do?needSimple=00&resourceType=2003&resourceId=${album_id}`;
        axios.get(info_url).then((response) => {
          const { data } = response;
          const info = {
  ...
  static mg_artist(url) {
    ...
    const target_url = `/api/appcnfmigu/MIGUM2.0/v1.0/content/singer_songs.do?pageNo=${page}&pageSize=${pageSize}&resourceType=2&singerId=${artist_id}`;
  ...
  static bootstrap_track(track, success, failure) {
    ...
    */
    ...
    const target_url = `/api/appcnfmigu/MIGUM2.0/strategy/listen-url/v2.2?netType=01&resourceType=E&songId=${songId}&toneFlag=${toneFlag}`;
    axios
      .get(target_url, {
        headers: {
          channel: '0146951',
  ...
  static search(url) {
    ...
    let searchSwitch = '';
    let target_url =
      '/api/jadeitemigu/music_search/v2/search/searchAll?';
    switch (searchType) {
      case '0':
  ...
  static get_playlist_filters() {
    return {
      success: (fn) => {
        let target_url =
          '/api/appcnfmigu/MIGUM3.0/v1.0/template/musiclistplaza-hottaglist/release';
        axios.get(target_url).then((response) => {
          ...
          target_url =
            '/api/appcnfmigu/MIGUM3.0/v1.0/template/musiclistplaza-taglist/release?templateVersion=1';
