...
async function getToken(refresh) {
  ...
  // let tokenString = await sessionStorage.getItem(kgKey);
  // if (tokenString && !refresh) {
  //   return JSON.parse(tokenString);
  // }
  const token_url = '/api/kwroot/root'
  await fetch(token_url)
  const token = cookie.get(kgKey)
  sessionStorage.setItem(kgKey, token)
  console.log(token)
  return token
}
...
export class kuwo {
  ...
  static kw_convert_song(item) {
    ...
    const track = {
      ...
      source: 'kuwo',
      source_url: `/api/kuwo/play_detail/${song_id}`,
      img_url: '',
  ...
  */
  static kw_convert_song2(item) {
    return {
      ...
      album_id: `kwalbum_${item.albumid}`,
      source: 'kuwo',
      source_url: `/api/kuwo/play_detail/${item.rid}`,
      img_url: item.pic,
  ...
  function kw_render_artist_result_item(index, item, params, callback) {
    const track = {
      ...
      album_id: `kwalbum_${item.albumid}`,
      source: 'kuwo',
      source_url: `/api/kuwo/play_detail/${item.musicrid}`,
      img_url: '',
  ...
  function kw_render_album_result_item(index, item, params, callback) {
    ...
    const track = {
      ...
      source: 'kuwo',
      source_url: `/api/kuwo/play_detail/${item.id}`,
      img_url: '',
  ...
  */
  static async kw_get_token(callback, isRetry) {
  ...
  static kw_render_tracks(url, page, callback) {
    ...
    switch (playlist_type) {
      case 'kwplaylist':
        ...
        tracks_url = `/api/kuwo/api/www/playlist/playListInfo?pid=${list_id}&pn=${page}&rn=100&httpsStatus=1`
        break
      case 'kwalbum':
        // tracks_url = `https://m.kuwo.cn/newh5app/api/mobile/v1/music/album/${list_id}?rn=1000`
        tracks_url = `/api/kuwo/api/www/album/albumInfo?albumId=${list_id}&pn=${page}&rn=100&httpsStatus=1`
        break
      default:
        break
    }
    ...
    this.kw_cookie_get(tracks_url, (response) => {
      const tracks = response.data.data.musicList.map((item) => this.kw_convert_song2(item))
  ...
  static search(url) {
    ...
    const target_url = `/api/kuwo/api/www/search/${api}?key=${keyword}&pn=${curpage}&rn=20`
    return {
      success: (fn) => {
        this.kw_cookie_get(target_url, (response) => {
          ...
          if (searchType === '0' && response.data.data !== undefined) {
            ...
            result = response.data.data.list.map((item) => ({
              ...
              title: this.html_decode(item.name),
              source: 'kuwo',
              source_url: `/api/kuwo/playlist_detail/${item.id}`,
              img_url: item.img,
  ...
  static bootstrap_track(track, success, failure) {
    ...
    // const target_url = '/api/antiserverkuwo/anti.s?' + `type=convert_url&format=mp3&response=url&rid=${song_id}`
    const target_url = `api/kuwo/api/v1/www/music/playUrl?mid=${song_id}&type=convert_url3&br=128kmp3`;
    /* const target_url = '/api/kuwo/url?'
    ...
    axios.get(target_url).then((response) => {
      ...
      if (data && data.data && data.data.url) {
        sound.url = data.data.url.replace(/(http):\/\/([a-z]+)\.(sycdn)\.(kuwo)\.cn/g, `${location.origin}/api/$2$3$4`)
        sound.platform = 'kuwo'
  ...
  static lyric(url) {
    // eslint-disable-line no-unused-vars
    const track_id = getParameterByName('track_id', url).replace('kwtrack_', '')
  ...
  static kw_artist(url) {
    ...
    return {
      success: (fn) => {
        let target_url = `/api/kuwo/api/www/artist/artist?artistid=${artist_id}`
        this.kw_cookie_get(target_url, (response) => {
          ...
          const info = {
            ...
            source_url: `/api/kuwo/singer_detail/${data.id}`
          }

          // Get songs
          target_url = `/api/kuwo/api/www/artist/artistMusic?artistid=${artist_id}&pn=1&rn=50`
      ...
      success: (fn) => {
        ...
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `/api/kuwo/album_detail/${data.albumid}`
          }
          // Get songs
  ...
  static show_playlist(url) {
    ...
    // const target_url = '/api/kuwo/www/categoryNew/getPlayListInfoUnderCategory?'
    // + `type=taglist&digest=10000&id=${37}&start=${offset}&count=50`;
    const target_url = `/api/kuwo/api/pc/classify/playlist/getRcmPlayList?pn=${
      offset / 25 + 1
    }&rn=25&order=hot&httpsStatus=1`
    /*
    精选歌单:roder=最热:hot, 最新:new
    tag歌单地址 /api/kuwo/api/pc/classify/playlist/getTagPlayList?pn=${offset / 25 + 1}&rn=25&id=37&httpsStatus=1
    id =华语:37,
    */
    return {
      success: (fn) => {
  ...
  static kw_get_playlist(url) {
    ...
    // /api/kuwo/api/www/playlist/playListInfo?pid=3134372426&pn=1&rn=30
    return {
      success: (fn) => {
        axios.get(target_url).then((response) => {
          const { data } = response
          ...
          const info = {
            ...
            source_url: `/api/kuwo/playlist_detail/${data.id}`
          }
          const { total } = data
          const page = Math.ceil(total / 100)
          const page_array = Array.from({ length: page }, (v, k) => k + 1)
